...
public class GlideCacheUtils {
    ...
    public String getCacheSize() {
        try {
            return getFormatSize(getFolderSize(new File(BaseApplication.getApplicationInstance().getCacheDir() + "/" + GlideCacheUtils.GLIDE_CARCH_DIR)));
        } catch (Exception e) {
            e.printStackTrace();
            return "获取失败";
        }
    ...
    public boolean cleanCatchDisk() {
        return deleteFolderFile(BaseApplication.getApplicationInstance().getCacheDir() + "/" + GlideCacheUtils.GLIDE_CARCH_DIR, true);
    }
        ...
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        Glide.get(BaseApplication.getApplicationInstance()).clearDiskCache();
                    }
                }).start();
            } else {
                Glide.get(BaseApplication.getApplicationInstance()).clearDiskCache();
    ...
    public boolean clearCacheMemory() {
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) { //只能在主线程执行
                Glide.get(BaseApplication.getApplicationInstance()).clearMemory();
                return true;
            }
        } catch (Exception e) {
            e.printStackTrace();
